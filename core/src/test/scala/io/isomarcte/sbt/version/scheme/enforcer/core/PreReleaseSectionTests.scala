...
final class PreReleaseSectionTests extends FunSuite {
  ...
  test("Component parsing") {
    ...
    assertEquals(PreReleaseSection.fromString("-SNAPSHOT.1"), Right(PreReleaseSection(Vector(PreReleaseVersionToken.unsafeFromString("SNAPSHOT"), PreReleaseVersionToken.unsafeFromBigInt(BigInt(1))))))
  }
}
