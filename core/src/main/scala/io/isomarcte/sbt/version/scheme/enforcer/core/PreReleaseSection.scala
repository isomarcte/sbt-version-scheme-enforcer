...
sealed abstract class PreReleaseSection extends Product with Serializable {
  ...
  /** The representation of the pre-release as derived from the components.
    ...
    */
  final def canonicalString: String = s"""-${value.map(_.value).mkString(".")}"""

  final override def toString: String = s"PreReleaseSection(${canonicalString})"
}

object PreReleaseSection {

  final private[this] case class PreReleaseSectionImpl(override val value: Vector[PreReleaseVersionToken])
      extends PreReleaseSection
  ...
  private[this] def errorMessage(message: String, input: String): String = s"${errorBaseMessage} ${message}: ${input}"

  /** Create a new [[PreReleaseSection]] from components. */
  def apply(value: Vector[PreReleaseVersionToken]): PreReleaseSection = PreReleaseSectionImpl(value)
  ...
  def fromString(value: String): Either[String, PreReleaseSection] =
    if (value === "-") {
      ...
      value
        .drop(1)
        .split(internal.separatorRegexString)
        .foldLeft(Right(Vector.empty[PreReleaseVersionToken]): Either[String, Vector[PreReleaseVersionToken]]) {
          case (acc, value) =>
            acc.flatMap(acc => PreReleaseVersionToken.fromString(value).map(value => acc ++ Vector(value)))
        }
        .fold(e => Left(errorMessage(s"Error parsing section. $e", value)), value => Right(apply(value)))
    } else {
    ...
    new Ordering[PreReleaseSection] {
      override def compare(x: PreReleaseSection, y: PreReleaseSection): Int = {
        ...
        x.value
          .map(Option.apply)
          .padTo(maxSize, None)
          .zip(y.value.map(Option.apply).padTo(maxSize, None))
          .foldLeft(0) {
            case (0, pair) =>
              pair match {
                case (Some(a), Some(b)) =>
