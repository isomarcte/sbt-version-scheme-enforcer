...
sealed abstract class MetadataSection extends Product with Serializable {
  ...
  final override def toString: String = s"MetadataSection(${canonicalString})"
}

object MetadataSection {
  final private[this] case class MetadataSectionImpl(override val value: Vector[MetadataVersionToken])
  ...
  def apply(value: Vector[MetadataVersionToken]): MetadataSection = MetadataSectionImpl(value)
  ...
  def fromString(value: String): Either[String, MetadataSection] =
    if (value === "+") {
      ...
      value
        ...
        .foldLeft(Right(Vector.empty): Either[String, Vector[MetadataVersionToken]]) { case (acc, value) =>
          acc.flatMap(acc => MetadataVersionToken.fromString(value).map(value => acc ++ Vector(value)))
        }
  ...
  implicit val orderingInstance: Ordering[MetadataSection] = Ordering.by(_.canonicalString)
}
