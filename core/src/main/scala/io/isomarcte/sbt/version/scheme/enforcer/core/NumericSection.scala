...
sealed abstract class NumericSection extends Product with Serializable {
  ...
  /** The representation of the numeric section as derived from the components.
    ...
    */
  final def canonicalString: String = value.map(_.value.toString).mkString(".")

  final override def toString: String = s"NumericSection(${canonicalString})"
}

object NumericSection {

  final private[this] case class NumericSectionImpl(override val value: Vector[NumericVersionToken])
      extends NumericSection
  ...
  def apply(value: Vector[NumericVersionToken]): NumericSection = NumericSectionImpl(value)
  ...
  def fromString(value: String): Either[String, NumericSection] =
    if (value.isEmpty) {
      ...
      value
        ...
        .toVector
        .foldLeft(Right(Vector.empty): Either[String, Vector[NumericVersionToken]]) { case (acc, value) =>
          acc.flatMap(acc =>
            Try(BigInt(value))
              .toEither
              .fold(
  ...
  implicit val ordering: Ordering[NumericSection] =
    new Ordering[NumericSection] {
      override def compare(x: NumericSection, y: NumericSection): Int = x.canonicalString.compare(y.canonicalString)
    }
