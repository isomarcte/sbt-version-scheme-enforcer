...
sealed abstract class NumericVersionToken extends Product with Serializable {
  ...
  final override def toString: String = s"NumericVersionToken(value = ${value})"
}

object NumericVersionToken {
  final private[this] case class NumericVersionTokenImpl(override val value: BigInt) extends NumericVersionToken
  ...
  /** Attempt to create [[NumericVersionToken]] from an `Int` value.
    *
    * The value must be >= 0.
    */
  def fromInt(value: Int): Either[String, NumericVersionToken] = fromBigInt(BigInt(value))

  /** Attempt to create [[NumericVersionToken]] from a `Long` value.
    *
    * The value must be >= 0.
    */
  def fromLong(value: Long): Either[String, NumericVersionToken] = fromBigInt(BigInt(value))
  ...
  /** As [[#fromInt]], but throws an exception if the value is invalid.
    ...
    * tests.
    */
  def unsafeFromInt(value: Int): NumericVersionToken =
  ...
  /** As [[#fromLong]], but throws an exception if the value is invalid.
    ...
    * tests.
    */
  def unsafeFromLong(value: Long): NumericVersionToken =
  ...
  implicit lazy val orderingInstance: Ordering[NumericVersionToken] = Ordering.by(_.value)
}
