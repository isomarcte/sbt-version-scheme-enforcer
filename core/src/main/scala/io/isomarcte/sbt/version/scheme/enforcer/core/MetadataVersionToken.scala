...
sealed abstract class MetadataVersionToken extends Product with Serializable {
  ...
  final override def toString: String = s"MetadataVersionToken(value = ${value})"
}

object MetadataVersionToken {
  final private[this] case class MetadataVersionTokenImpl(override val value: String) extends MetadataVersionToken
  ...
  implicit val orderingInstance: Ordering[MetadataVersionToken] = Ordering.by(_.value)
}
